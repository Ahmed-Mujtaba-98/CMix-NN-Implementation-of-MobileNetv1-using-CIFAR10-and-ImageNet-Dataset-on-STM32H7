...
arm_status
arm_fully_connected_asym_uint8_PACT_icn(const uint8_t * pV,
                           	   	   const uint8_t * pM,
								   ...
								   int16_t * vec_buffer)
{
    ...
    while (rowCnt)
    {
        ...
        int16_t VzA2[2] = {z_wt[i+1],z_wt[i+1]};
        ...
        while (colCnt)
        {
            ...
            inM21 = __SSUB16(inM21, inzA2);
            sum = __SMLAD(inV, inM11, sum);
            sum2 = __SMLAD(inV, inM21, sum2);

            inV = *__SIMD32(pA)++;
            inM12 = __SSUB16(inM12, inzA);
            inM22 = __SSUB16(inM22, inzA2);
		...
		sum  = ((__HI_SMULL(sum,m_zero[i])) >> n_zero[i]) + z_out;	//i=0
		sum2 = ((__HI_SMULL(sum2,m_zero[i+1])) >> n_zero[i+1]) + z_out;	//i=1
		i=i+2;
